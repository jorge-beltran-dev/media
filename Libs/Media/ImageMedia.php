 ...
 */
	...
	var $adapters = array('Imagick', 'Gd', 'ImagickShell');
 ...
 */
	function fitInside($width, $height) {
		list($currentWidth, $currentHeight) = $this->dimensions();
		$rx = $currentWidth / $width;
		$ry = $currentHeight / $height;
 ...
 */
	function ratio($known = true) {
		...
		return $this->_knownRatio($currentWidth, $currentHeight);
	}
 ...
 */
	function megapixel() {
		list($currentWidth, $currentHeight) = $this->dimensions();
		return (integer)($currentWidth * $currentHeight / 1000000);
	}
	...
	function _normalizeDimensions($width, $height, $set = 'ratio') {
		...
		if ($set == 'maximum') {
			...
			if (empty($width)) {
				...
				$width = $ratio * $currentWidth;
			}
			if (empty($height)) {
				$ratio = $width / $currentWidth;
				$height = $ratio * $currentHeight;
			}
		}
